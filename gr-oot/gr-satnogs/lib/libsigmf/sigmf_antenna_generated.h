...
struct GlobalBuilder {
  ...
  {
    fbb_.AddElement<float>(Global::VT_HORIZONTAL_BEAM_WIDTH, horizontal_beam_width,
                           0.0f);
  }
  void add_vertical_beam_width(float vertical_beam_width)
  {
    fbb_.AddElement<float>(Global::VT_VERTICAL_BEAM_WIDTH, vertical_beam_width,
                           0.0f);
  }
  void add_cross_polar_discrimination(float cross_polar_discrimination)
  {
    fbb_.AddElement<float>(Global::VT_CROSS_POLAR_DISCRIMINATION,
                           cross_polar_discrimination, 0.0f);
  }
  void add_voltage_standing_wave_ratio(float voltage_standing_wave_ratio)
  {
    fbb_.AddElement<float>(Global::VT_VOLTAGE_STANDING_WAVE_RATIO,
                           voltage_standing_wave_ratio, 0.0f);
  }
                             ...
                             const flatbuffers::resolver_function_t *_resolver) const
{
  ...
  {
    auto _e = low_frequency();
    _o->low_frequency = _e;
  };
  {
    auto _e = high_frequency();
    _o->high_frequency = _e;
  };
  {
    auto _e = gain();
    _o->gain = _e;
  ...
  {
    auto _e = horizontal_beam_width();
    _o->horizontal_beam_width = _e;
  };
  {
    auto _e = vertical_beam_width();
    _o->vertical_beam_width = _e;
  };
  {
    auto _e = cross_polar_discrimination();
    _o->cross_polar_discrimination = _e;
  };
  {
    auto _e = voltage_standing_wave_ratio();
                                 ...
                                 const flatbuffers::resolver_function_t *_resolver) const
{
  ...
  (void)_resolver;
  {
    auto _e = azimuth_angle();
    _o->azimuth_angle = _e;
  };
  {
    auto _e = elevation_angle();
    _o->elevation_angle = _e;
  };
  {
    auto _e = polarization();
    if (_e) {
      _o->polarization = _e->str();
    }
  };
}
  ...
  const flatbuffers::rehasher_function_t *_rehasher)
{
  ...
  auto _polarization = _o->polarization.empty() ? 0 : _fbb.CreateString(
                         _o->polarization);
  return antenna::CreateAnnotation(
           _fbb,
           _azimuth_angle,
                            ...
                            const flatbuffers::resolver_function_t *_resolver) const
{
  ...
  (void)_resolver;
  {
    auto _e = global();
    if (_e) {
      _o->global = std::unique_ptr<GlobalT>(_e->UnPack(_resolver));
    }
  };
  {
    auto _e = annotation();
    if (_e) {
      _o->annotation = std::unique_ptr<AnnotationT>(_e->UnPack(_resolver));
    }
  };
  {
    auto _e = capture();
    if (_e) {
      _o->capture = std::unique_ptr<CaptureT>(_e->UnPack(_resolver));
    }
  };
}
