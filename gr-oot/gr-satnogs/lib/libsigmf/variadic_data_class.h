...
public:
  ...
  T &access()
  {
    return MatchingField<0, T, sigmftypes, SigMFNamespaceOfType<0, T>::value>::get(
             sigmf_namespaces);
  }
...
private:
  ...
  struct TupleIterator {
    ...
    static std::string get_namespace(const T *ttype)
    {
      ...
      int status;
      demangled_name = abi::__cxa_demangle(mangled_name.c_str(), demangled_name,
                                           &size_of_demangled_name,
                                           &status);
    ...
    static json to_json(const Tuple &tp, bool include_defaults)
    {
      ...
      json existing_vals =
        TupleIterator<next, Tuple, ReachedEnd<next, size>::value>::to_json(tp,
            include_defaults);
    ...
    static void from_json(Tuple &tp, const json j)
    {
      ...
      dumb_ripoff_visitor.fbb.Finish(flatbuffers::Offset<void>
                                     (dumb_ripoff_visitor._stop));

      auto *annoptr =
        flatbuffers::GetRoot<typename std::tuple_element<Index, sigmftypes>::type::TableType>
        (
  ...
  struct SigMFNamespaceOfType :
    SameType<T, typename std::tuple_element<N, sigmftypes>::type> {
  };
  ...
  struct MatchingField {
    ...
    {
      return MatchingField < N + 1, T, Tuple, SigMFNamespaceOfType < N + 1,
             T >::value >::get(tp);
    }
  };
