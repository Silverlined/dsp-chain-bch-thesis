...
def plotWelchPSD(x,fs,fc,ax = None,color='b', label=None):
    ...
    indices = (f>=-3*fc) & (f<3*fc)   # To plot PSD from Fc to 4*Fc
    Pxx = Pxx[:]/Pxx[:][0] # normalized psd w.r.t Fc
    ax.plot(f[:]-fc,10*log10(Pxx),color,label=label) #Plot in the given axes
