...
N_bits = 4096;               % Number of bits used for timing error analysis
Rb = 9600.0;
bits_per_sym = 1;
...
a_sps = 2;              % actual samples per symbol used in application
...
% Convert symbols to baseband pulses, pulse shaping
pkg load signal;
...
isi_sps = round((tea_sps/a_sps * 4) - tea_sps); % Play with isi samples to see S-curve effects
if (mod(isi_sps, 2) != 0)
    isi_sps = isi_sps + 1;
end
